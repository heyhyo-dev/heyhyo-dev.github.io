...
> 비즈니스 관점에서 “하나의 업무 단위”  
> Sales Order(판매오더): 헤더 + 아이템 + 스케줄라인 + 파트너 + 가격조건  
> Purchase Order(구매오더): 헤더 + 아이템 + 납품일정 + 계정지정  
...
> 자동차는 엔진(Header), 바퀴(Item), 옵션(Supplement) 등이 유기적으로 연결되어 있으며, '시동 걸기', '가속하기'와 같은 동작(Behavior)을 함께 가지고 있다.  
